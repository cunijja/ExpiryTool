...
for d in (
    DATA_DIR,
    REPORTS_DIR,
    REPORTS_PICKLISTS_DIR,
    REPORTS_LOW_STOCK_DIR,
    REPORTS_EXPIRIES_DIR,
):
    ...
    with get_db() as con:
        ...
        user_col = (
            ...
            if "user" in cols
                ...
                else "who" if "who" in cols else None
            )
        )
...
def screen_audit_log(stdscr):
    ...
    try:
        while True:
            ...
            help_line = (
                "[Enter] View   [D] Delete   [R] Refresh   [P] Purge All   [F2] Back"
            )
            ...
            sel = list_paginated(
                stdscr, "AUDIT LOG", rows, _render_log_row, page_size=14
            )
            ...
            while True:
                ...
                if k in (curses.KEY_F2, 27):  # back to previous menu
                    ...
                    if confirm_dialog(
                        stdscr, "Delete entry", f"Delete log #{r['id']}?"
                    ):
                        with no_audit():
...
def dao_user_list():
    with get_db() as con:
        rows = con.execute(
            "SELECT id, initials, is_admin FROM users ORDER BY initials"
        ).fetchall()
        return [dict(r) for r in rows]
...
def dao_user_get(user_id: int):
    with get_db() as con:
        r = con.execute(
            "SELECT id, initials, pw_hash, is_admin FROM users WHERE id=?", (user_id,)
        ).fetchone()
...
def dao_user_get_by_initials(initials: str):
    with get_db() as con:
        r = con.execute(
            "SELECT id, initials, pw_hash, is_admin FROM users WHERE UPPER(initials)=UPPER(?)",
            (initials.strip(),),
        ).fetchone()
        return dict(r) if r else None
...
def dao_user_create(initials: str, password: str, is_admin: bool = False) -> int:
    ...
    with get_db() as con:
        con.execute(
            "INSERT INTO users (initials, pw_hash, is_admin) VALUES (?,?,?)",
    ...
    log_action(
        ...
        "user_create",
        f"init={initials}, admin={is_admin}",
    )
    return user_id
...
def dao_user_update(user_id: int, initials: str = None, is_admin: bool = None):
    ...
    if initials is not None:
        ...
        args.append(initials.strip().upper())
...
def dao_user_set_password(user_id: int, password: str):
    with get_db() as con:
        con.execute(
            "UPDATE users SET pw_hash=? WHERE id=?", (hash_pw(password), user_id)
        )
...
def dao_user_delete(user_id: int):
    ...
    log_action(
        CURRENT_USER.get("initials", ""),
        "user_delete",
        f"id={user_id} ({u['initials'] if u else ''})",
    )
...
def init_db():
    with db() as con:
        ...
        if cur.fetchone()["c"] == 0:
            con.execute(
                "INSERT INTO users(initials,password,is_admin) VALUES('CB','CB',1)"
            )
            con.execute(
                "INSERT INTO logs(ts,who,action,details) VALUES(?,?,?,?)",
                (
...
def dao_item_upsert(
    ...
    with db() as con:
        ...
        if row:
            ...
            con.execute(
                """INSERT INTO items(brand,name,upc,size,uom,vendor,cost,retail,notes,reorder_level)
                           VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (
                    brand,
                    name,
...
def dao_item_stock(item_id: int) -> int:
    ...
    with db() as con:
        row = con.execute(
            "SELECT COALESCE(SUM(qty),0) AS q FROM lots WHERE item_id=?", (item_id,)
        ).fetchone()
...
def dao_item_stock_qty(item_id):
    with db() as con:
        row = con.execute(
            "SELECT COALESCE(SUM(qty),0) q FROM lots WHERE item_id=?", (item_id,)
        ).fetchone()
    return int(row["q"] or 0)
...
def _query_items_by_desc(q):
    return dao_search_by_desc(q)


def _query_items_by_upc(q):
    return dao_search_by_upc(q)


def _query_items_by_expiry_week(
    s,
):  # needs _add_days_yyyymmdd which already exists in your file
...
def render_picklist_report_lines(picklist_header: dict, rows: list):
    ...
    lines.append(
        f"Name: {picklist_header.get('name', '')}   "
    ...
    lines.append("-" * 64)
    lines.append(
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  (v{APP_VERSION})"
    )
    return lines
...
def render_low_stock_report_lines(rows: list):
    ...
    lines.append("-" * 64)
    for r in rows:
        stk = int(r.get("stock", 0))
        rl = int(r.get("reorder_level", 0))
        lines.append(
            f"{stk:>4}/{rl:<4} {('#' + str(r.get('id', ''))):>5}  {(r.get('brand') or ''):<16}  {(r.get('name') or ''):<28}"
        )
    lines.append("-" * 64)
    lines.append(
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  (v{APP_VERSION})"
...
def render_expiries_report_lines(title: str, rows: list):
    ...
    for r in rows:
        lines.append(
            f"{fmt_ymd(r.get('expiry', '')):<12} {str(r.get('qty', 0)):>4}  {(r.get('brand') or ''):<16}  {(r.get('name') or ''):<28}"
        )
        if r.get("lot"):
...
def confirm_dialog(
    stdscr, title="Confirm", message="Are you sure?", yes="Yes", no="No"
):
    init_theme(stdscr)
...
def _render_fields(stdscr, top, left, box_w, fields, idx, mask):
    ...
    for i, (label, value) in enumerate(fields):
        ...
        is_masked = bool(
            mask and label in (mask if isinstance(mask, (set, list, tuple)) else {mask})
        )
...
def review_item_dialog(stdscr, data: dict):
    ...
    stdscr.addstr(
        top + bh - 2, left + 2, "[Tab] Save   [Enter] Edit   [F2] Back", curses.A_DIM
    )
    stdscr.refresh()
...
def list_paginated(stdscr, title, rows, line_func, page_size=12):
    ...
    while True:
        ...
        stdscr.addstr(
            top + 1,
            left + 2,
            "[Up/Down] Move  [Enter] Select  [N] Next  [P] Prev  [F2] Back",
            curses.A_DIM,
        )
        ...
        for i, r in enumerate(slice_rows):
            ...
            stdscr.addstr(
                top + 3 + i, left + 3, str(text)[: box_w - 6].ljust(box_w - 6), attr
            )
...
def dao_item_by_id(item_id: int):
    with db() as con:
        r = con.execute(
            """SELECT id, brand, name, upc, size, uom, vendor,
...
def dao_picklist_create(name: str):
    with db() as con:
        con.execute(
            """INSERT INTO picklists(name,status,created_at,created_by)
                       VALUES(?,?,?,?)""",
            (
                name,
                "open",
...
def dao_picklist_toggle(pid: int):
    ...
    log_action(
        CURRENT_USER.get("initials", ""), "picklist_toggle", f"id={pid};{new_status}"
    )
    return True
...
def dao_picklist_add_item(pid: int, item_id: int, qty: int = 0, note: str = None):
    ...
    log_action(
        CURRENT_USER.get("initials", ""),
        "picklist_add_item",
        f"pid={pid};item={item_id};qty={qty}",
    )
    return True
...
def dao_picklist_remove_item(pid: int, item_id: int):
    with db() as con:
        con.execute(
            "DELETE FROM picklist_items WHERE picklist_id=? AND item_id=?",
            (pid, item_id),
        )
    log_action(
        CURRENT_USER.get("initials", ""),
        "picklist_remove_item",
        f"pid={pid};item={item_id}",
    )
    return True
...
def build_picklist_from_low_stock(pid: int, add_qty: bool = False):
    ...
    log_action(
        CURRENT_USER.get("initials", ""),
        "picklist_build_low_stock",
        f"pid={pid};count={len(lows)}",
    )
    return len(lows)
...
def save_picklist_print(pid: int):
    ...
    log_action(
        CURRENT_USER.get("initials", ""),
        "report_picklist_saved",
        f"pid={pid};file={os.path.basename(path)}",
    )
    return path
...
def save_expiry_report(bucket_key: str):
    ...
    out = [
        title,
        sub,
        f"Window: {fmt_ymd(start)} .. {fmt_ymd(end)}",
        sep,
        " | ".join(_fw(h, w, "center") for h, w in cols),
        sep,
    ]
...
def _prompt_note(stdscr, title="NOTE", seed=""):
    f = input_form(
        stdscr, title, [("Note (optional)", seed)], footer="[Enter] OK   [F2] Cancel"
    )
    if f is None:
        return None
...
def screen_picklist_items(stdscr, pid: int, header: str):
    ...
    while True:
        rows = dao_picklist_items(pid)
        init_theme(stdscr)
        title_bar(stdscr, path_to_str(CURRENT_PATH), f"PICKLIST #{pid} — ITEMS")
        h, w = stdscr.getmaxyx()
        ...
        # table header
        stdscr.addstr(
            top + 3, left + 2, "   QTY   [STCK]   BRAND / NAME / SIZE UOM   — NOTE"
        )
        # list
        cursor = 0
        while True:
            ...
            for i, r in enumerate(rows[: box_h - 8]):
                attr = (
                    curses.color_pair(PALETTE["select"])
                    if i == cursor
            ...
            stdscr.addstr(
                top + box_h - 2,
                left + 2,
                "[Enter]=Edit Qty/Note for highlighted  |  [F2] Back",
                curses.A_DIM,
            )
            stdscr.refresh()
            ...
            if k == curses.KEY_F2:
                ...
                row = rows[cursor]
                qty = _prompt_qty(
                    stdscr, f"SET QTY — #{row['item_id']}", str(row.get("qty", 0))
                )
                if qty is None:
                    continue
                ...
                row = rows[cursor]
                note = _prompt_note(
                    stdscr, f"SET NOTE — #{row['item_id']}", row.get("note") or ""
                )
                if note is None:
                    continue
                with db() as con:
                    con.execute(
                        "UPDATE picklist_items SET note=? WHERE id=?",
                        (note, row["pli_id"]),
                    )
                log_action(
                    CURRENT_USER.get("initials", ""),
                    "picklist_set_note",
                    f"pid={pid};item={row['item_id']}",
                )
                ...
                if confirm_dialog(
                    stdscr,
                    "Remove Item",
                    f"Remove {row.get('brand', '')} {row.get('name', '')}?",
                ):
                ...
                if confirm_dialog(
                    stdscr,
                    "Build from Low Stock",
                    "Add suggested quantities to reach minimum?",
                ):
                ...
                info_center(
                    stdscr,
                    "PICKLIST SAVED",
                    f"Saved to:\n{path}\n\nYou can open/print the TXT file.",
                    "[Enter] OK   [F2] Back",
                )
...
def open_picklist(stdscr, pid: int):
    ...
    try:
        ...
        _ = list_paginated(
            stdscr, title, rows, _render, page_size=16
        )  # read-only; F2 backs out
    finally:
        pop_path()
...
def screen_picklists(stdscr):
    ...
    try:
        ...
        while True:
            ...
            init_theme(stdscr)
            title_bar(stdscr, path_to_str(CURRENT_PATH), "PICK LISTS")
            h, w = stdscr.getmaxyx()
            ...
            help_line = (
                "[Up/Down] Move  [Enter] Open  [N] New  [T] Toggle  [D] Delete  "
            ...
            # --- Build/Export actions ---
                ...
                ord("b"),
                ord("B"),
            ):  # build from Low Stock into selected/open picklist
                if rows:
                    pid = rows[cursor]["id"]
                ...
                ord("e"),
                ord("E"),
            ):  # build from Expiries using your interactive ranges
                if rows:
                    pid = rows[cursor]["id"]
...
def screen_month_view(stdscr):
    ...
    try:
        while True:
            ...
            title_bar(
                stdscr,
                path_to_str(CURRENT_PATH),
                f"MONTH VIEW – {first.strftime('%B %Y')}",
            )
            stdscr.addstr(
                3,
                2,
                "[←/→] Day  [↑/↓] Week  [N] Next month  [P] Prev month  [Enter] Open  [F2] Back",
                curses.A_DIM,
            )
    ...
    try:
        while True:
            ...
            for r in rows:
                ...
                labels.append(
                    f"{upc}  {brand} {name}  [Lot {lot}]  qty:{qty}  exp:{exp}"
                )
            ...
            while True:
                ...
                if choice == 3:
                    if confirm_dialog(
                        stdscr, "DELETE LOT", "Remove all from this lot?"
                    ):
...
def _attr_red():
    _ensure_extra_pairs()
    return curses.color_pair(90) | curses.A_BOLD


def _attr_yellow():
    _ensure_extra_pairs()
    return curses.color_pair(91) | curses.A_BOLD


def _attr_green():
    _ensure_extra_pairs()
    return curses.color_pair(92) | curses.A_BOLD
...
def expiry_attr_for_week_view(yyyymmdd: str):
    ...
    if dd == 0:
        return _attr_red()  # today
    if 1 <= dd <= 7:
        return _attr_yellow()  # next 7 days
    return curses.A_NORMAL
...
def expiry_attr_for_month_view(yyyymmdd: str):
    ...
    if dd is None:
        return curses.A_NORMAL
    if dd <= 14:
        return _attr_yellow()  # next 2 weeks
    if dd <= 28:
        return _attr_green()  # next 4 weeks
    return curses.A_NORMAL
...
def report_low_stock_txt():
    ...
    path = os.path.join(
        REPORTS_LOW_STOCK_DIR, f"low_stock_{datetime.date.today():%Y%m%d}.txt"
    )
...
def report_low_stock_pdf():
    ...
    try:
        ...
        c.drawString(54, y, "LOW STOCK REPORT")
        y -= 16
        c.setFont("Helvetica", 10)
        c.drawString(54, y, f"Generated: {datetime.datetime.now():%Y-%m-%d %H:%M}")
        y -= 18
        c.setFont("Helvetica-Bold", 10)
        c.drawString(54, y, "ID  BRAND  NAME  SIZE UOM  UPC        STOCK  REORDER")
        y -= 14
        c.setFont("Helvetica", 10)
        for r in rows:
            line = (
...
def report_expiries_pdf(start_ymd: str, end_ymd: str):
    ...
    try:
        ...
        y = height - 54
        c.setFont("Helvetica-Bold", 14)
        c.drawString(54, y, f"EXPIRIES {fmt_ymd(start_ymd)} – {fmt_ymd(end_ymd)}")
        y -= 16
        c.setFont("Helvetica", 10)
        c.drawString(54, y, f"Generated: {datetime.datetime.now():%Y-%m-%d %H:%M}")
        y -= 18
        c.setFont("Helvetica-Bold", 10)
        c.drawString(54, y, "EXPIRY      QTY  UPC        BRAND  NAME  SIZE UOM  LOT")
        y -= 14
        c.setFont("Helvetica", 10)
        for r in rows:
            line = (
...
def screen_item_details(stdscr, item_id: int):
    ...
    try:
        ...
        stdscr.addstr(
            y,
            x,
            f"#{it.get('id', '')}  {(it.get('brand') or '')}  {(it.get('name') or '')}",
        )
        y += 1
        stdscr.addstr(
            y,
            x,
            f"UPC: {it.get('upc', '') or ''}    Vendor: {it.get('vendor', '') or ''}",
        )
        y += 1
        cost = it.get("cost") or 0.0
        retail = it.get("retail") or 0.0
        stdscr.addstr(y, x, f"Cost: {cost:0.2f}    Retail: {retail:0.2f}")
        y += 1
        stdscr.addstr(
            y,
            x,
            f"Reorder Level: {it.get('reorder_level') or 0}    Current Stock: {dao_item_stock_qty(item_id)}",
        )
        y += 1
        stdscr.addstr(y, x, f"Notes: {it.get('notes') or ''}")
        y += 2

        # lots
        stdscr.hline(y, x, curses.ACS_HLINE, bw - 4)
        y += 1
        stdscr.addstr(y, x, "Lots (by expiry)")
        y += 1
        stdscr.addstr(y, x, " " + "Lot".ljust(16) + "Expiry".ljust(14) + "Qty")
        y += 1
        ...
        while True:
            ...
            if k == curses.KEY_F2:
                ...
                if confirm_dialog(
                    stdscr, "DELETE ITEM", "Delete this item and all its lots?"
                ):
                    dao_item_delete(item_id)
                    info_toast(stdscr, "Item deleted.")
                    log_action(
                        CURRENT_USER.get("initials", ""), "item_delete", f"id={item_id}"
                    )
                    return
        ...
        while True:
            ...
            if idx == 0:
                ...
                if not rows:
                    ...
                    continue
                sel = list_paginated(
                    stdscr, "RESULTS", rows, _render_item_line, page_size=12
                )
                if sel >= 0:
...
def screen_add_item(stdscr):
    ...
    try:
        ...
        while True:
            ...
            try:
                cost = float((d.get("Cost", "") or "0").strip())
            except:
                cost = 0.0
            try:
                retail = float((d.get("Retail", "") or "0").strip())
            except:
                retail = 0.0
            try:
                reorder = int((d.get("Reorder Level", "") or "0").strip())
            except:
                reorder = 0
...
def screen_regular_expiry(stdscr):
    ...
    try:
        ...
        f = input_form(
            stdscr,
            f"ADD EXPIRY — #{it['id']} {it['brand']} {it['name']}",
            fields,
        ...
        log_action(
            CURRENT_USER.get("initials", ""),
            "expiry_add",
            f"id={item_id};exp={expiry};qty={qty}",
        )
...
def screen_decode(stdscr):
    ...
    try:
        ...
        f = input_form(
            stdscr,
            f"DECODE LOT — #{it['id']} {it['brand']} {it['name']}",
            [("Lot", "")],
        ...
        options = [f"Use decoded date: {fmt_ymd(c)}" for c in cand] + [
            "Enter date manually"
        ]
        ...
        while True:
            ...
            if k in (curses.KEY_F2, 27):
                ...
                if u["is_admin"] and admins <= 1:
                    ...
                    if confirm_dialog(
                        stdscr, "Delete user", f"Delete {u['initials']}?"
                    ):
...
def dao_remove_qty_from_lot(item_id: int, lot: str | None, qty: int) -> int:
    ...
    with get_db() as con:
        ...
        if new_qty <= 0:
            con.execute(
                "DELETE FROM lots WHERE item_id=? AND IFNULL(lot,'')=?",
                (item_id, lot_key),
            )
        else:
            con.execute(
...
def ensure_picklist_items_table():
    ...
    with db() as con:
        ...
        cols = [
            r["name"]
            for r in con.execute("PRAGMA table_info(picklist_items)").fetchall()
        ]
        if "qty" not in cols:
            con.execute(
                "ALTER TABLE picklist_items ADD COLUMN qty INTEGER NOT NULL DEFAULT 0"
            )
...
def dao_picklist_items_list(pid: int):
    ...
    with db() as con:
        ...
        qty_col = (
            "li.qty"
            if "qty" in cols
            else ("li.quantity" if "quantity" in cols else "1")
        )
        sql = f"""
...
def login_screen(stdscr):
    ...
    while True:
        ...
        with db() as con:
            row = con.execute(
                "SELECT initials,password,is_admin FROM users WHERE initials=?", (user,)
            ).fetchone()
            ...
            if choice == 0:
                if "screen_picklists" in globals():
                    ...
                    info_center(
                        stdscr,
                        "PICK LISTS",
                        "screen_picklists() not found in this build.",
                    )
    finally:
        pop_path()
    ...
    try:
        while True:
            ...
            if choice == 0:
                ...
                info_toast(
                    stdscr,
                    f"Saved {'PDF' if ok_pdf else 'TXT (no PDF engine)'}: {os.path.basename(path)}",
                )
...
def auth_login(stdscr) -> bool:
    ...
    try:
        ...
        while True:
            ...
            info_center(
                stdscr, "LOGIN FAILED", "Check initials/password and try again."
            )
    finally:
        pop_path()
